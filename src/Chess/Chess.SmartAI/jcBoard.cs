...
namespace Chess.SmartAI
{
    public class jcBoard
    {
        ...
        public const int WHITE_PAWN = PAWN + jcPlayer.SIDE_WHITE;
        public const int WHITE_KNIGHT = KNIGHT + jcPlayer.SIDE_WHITE;
        public const int WHITE_BISHOP = BISHOP + jcPlayer.SIDE_WHITE;
        public const int WHITE_ROOK = ROOK + jcPlayer.SIDE_WHITE;
        public const int WHITE_QUEEN = QUEEN + jcPlayer.SIDE_WHITE;
        public const int WHITE_KING = KING + jcPlayer.SIDE_WHITE;
        public const int BLACK_PAWN = PAWN + jcPlayer.SIDE_BLACK;
        public const int BLACK_KNIGHT = KNIGHT + jcPlayer.SIDE_BLACK;
        public const int BLACK_BISHOP = BISHOP + jcPlayer.SIDE_BLACK;
        public const int BLACK_ROOK = ROOK + jcPlayer.SIDE_BLACK;
        public const int BLACK_QUEEN = QUEEN + jcPlayer.SIDE_BLACK;
        public const int BLACK_KING = KING + jcPlayer.SIDE_BLACK;
        ...
        public const int ALL_WHITE_PIECES = ALL_PIECES + jcPlayer.SIDE_WHITE;
        public const int ALL_BLACK_PIECES = ALL_PIECES + jcPlayer.SIDE_BLACK;
        ...
        public jcBoard()
        {
            ...
            CastlingStatus = new bool[4];
            HasCastled = new bool[2];
        ...
        public bool Print()
        {
            for (int line = 0; line < 8; line++)
            {
                ...
                for (int col = 0; col < 8; col++)
                {
                    ...
                    // Show the piece
                    Console.Write("| " + PieceStrings[piece] + " ");
                }
                Console.WriteLine("|");
                Console.WriteLine("|    |    |    |    |    |    |    |    |");
            }
            ...
            if (CurrentPlayer == jcPlayer.SIDE_BLACK)
                Console.WriteLine("NEXT MOVE: BLACK ");
            else
                Console.WriteLine("NEXT MOVE: WHITE");
        ...
        public bool Load(IFenData fenData)
        {
            ...
            {
                var whichPieceStr = Helper.GetPieceString(pieceInfo);
                var whichSquare = Helper.GetPieceSquare(pieceInfo);

                var whichPiece = 0;
                while (!whichPieceStr.Equals(PieceStrings[whichPiece], StringComparison.CurrentCultureIgnoreCase))
                    whichPiece++;
            ...
            //for (int i = 0; i < 4; i++)
            //{
            //    if ("TRUE".Equals(sr.ReadLine(), StringComparison.CurrentCultureIgnoreCase))
            //        SetCastlingStatus(i, true);
            //    else
            //        SetCastlingStatus(i, false);
            //}

            //TODO: SetEnPassantPawn(Convert.ToInt64(sr.ReadLine()));
        ...
        public bool Save(String fileName)
        {
            ...
            {
                // Whose turn is it?
                sw.Write(jcPlayer.PlayerStrings[CurrentPlayer]);
                sw.WriteLine();
                ...
                for (int piece = 0; piece < ALL_PIECES; piece++)
                {
                    for (int square = 0; square < ALL_SQUARES; square++)
                    {
                        ...
                        {
                            sw.Write(PieceStrings[piece] + " " + square);
                            sw.WriteLine();
                        }
                    }
                }

                // And constly, dump the castling status and the en passant pawn
                for (int i = 0; i < 4; i++)
                {
                    if (CastlingStatus[i])
                        sw.Write("TRUE");
        ...
        // private bool SetCastlingStatus
        ...
        private bool SetEnPassantPawn(long bitboard)
        {
        ...
        // private bool SetCurrentPlayer
        // Whose turn is it?
        public bool SetCurrentPlayer(int which)
        {
            CurrentPlayer = which;
            return true;
        }
